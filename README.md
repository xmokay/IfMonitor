...
- Tray icon blinks between alert tints while any monitored adapter is unhealthy; returns to normal when all are up (no click-to-dismiss required)
...
| File | Purpose |
|------|---------|
| `icon.png` | Source artwork (256×256, **true** alpha); generated by `tools/BuildIcon` — bold bars, ~84% canvas fill |
...
|-------|------------------|
| **Tray** (taskbar overflow) | Runtime `IconArtwork` — three green bars |
